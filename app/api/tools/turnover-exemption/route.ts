...
function buildDeclarationXml(
  ...
  return `
<!-- BIDAXIS-TURNOVER-EXEMPTION-START -->
...
${xmlParagraph(
  `Subject: Turnover Exemption Declaration against Bid No. ${data.bidNumber}`,
  {
    bold: true,
    after: 220,
  }
...
${xmlParagraph(
  `We request consideration of exemption from the applicable turnover eligibility requirement on the basis of: ${data.exemptionBasis}.`,
  {
    after: 180,
  }
)}
...
${xmlParagraph(
  `The Registration / Certificate Number furnished in support of the above exemption claim is ${data.registrationNumber}.`,
  {
    after: 180,
  }
)}
...
${xmlParagraph(
  "We understand that the applicability of the claimed turnover exemption is subject to the eligibility provisions, documentary requirements and other conditions specified in the referenced bid / tender.",
  {
    after: 180,
  }
)}
...
${xmlParagraph(
  "We understand that submission of this declaration by itself does not establish eligibility for exemption and that the buyer / procuring authority may verify the registration, certificate and other supporting documents submitted by us.",
  {
    after: 180,
  }
)}
...
${xmlParagraph(
  "We confirm that the information furnished in this declaration is true and correct to the best of our knowledge and belief and that the supporting documents submitted by us correspond with the exemption basis stated above.",
  {
    after: 300,
  }
)}
...
<!-- BIDAXIS-TURNOVER-EXEMPTION-END -->
`;
}
...
function removePreviousDeclaration(
  ...
  return xml.replace(
    /<!-- BIDAXIS-TURNOVER-EXEMPTION-START -->[\s\S]*?<!-- BIDAXIS-TURNOVER-EXEMPTION-END -->/g,
    ""
  );
}
    ...
    sections: [
      {
        ...
        children: [
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: "TURNOVER EXEMPTION DECLARATION",
                bold: true,
                size: 26,
              }),
            ],
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text:
                  `Subject: Turnover Exemption Declaration against Bid No. ` +
                  `${data.bidNumber}`,
                bold: true,
                size: 22,
              }),
          ...
          bodyParagraph(
            `We request consideration of exemption from the applicable ` +
              `turnover eligibility requirement on the basis of: ` +
              `${data.exemptionBasis}.`
          ),

          bodyParagraph(
            `The Registration / Certificate Number furnished in support ` +
              `of the above exemption claim is ${data.registrationNumber}.`
          ),

          bodyParagraph(
            "We understand that the applicability of the claimed turnover " +
              "exemption is subject to the eligibility provisions, " +
              "documentary requirements and other conditions specified " +
              "in the referenced bid / tender."
          ),

          bodyParagraph(
            "We understand that submission of this declaration by itself " +
              "does not establish eligibility for exemption and that the " +
              "buyer / procuring authority may verify the registration, " +
              "certificate and other supporting documents submitted by us."
          ),

          bodyParagraph(
            "We confirm that the information furnished in this declaration " +
              "is true and correct to the best of our knowledge and belief " +
              "and that the supporting documents submitted by us correspond " +
              "with the exemption basis stated above.",
            280
          ),
...
async function createLetterheadDocument(
  ...
  /*
   * Keep the final section properties in place.
   * This preserves header/footer references and the
   * document's existing page setup.
   */
  ...
  try {
    ...
    if (!accuracyAccepted) {
      return Response.json(
        {
          error:
            "Please confirm the accuracy of the turnover exemption information before generating the document.",
        },
        {
          status: 400,
        }
    ...
    const filename =
      `Turnover-Exemption-Declaration-${cleanFilename(
        data.companyName
      )}.docx`;
    ...
    console.error(
      "Turnover Exemption generation error:",
      error
    );
    ...
    return Response.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Unable to generate Turnover Exemption Declaration.",
      },
      {
        status: 500,
      }
